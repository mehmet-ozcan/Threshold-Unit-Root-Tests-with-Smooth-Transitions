#########################################################################################################
# Empirical Size simulation Codes of New Combined Threshold Unit Root Test introduced in
# 
...
# Size DGP
CHSizeDGP <- function(n,a){
    n <- n+1
    et<- as.matrix(rnorm(n))
    et<- et-mean(et)

    dyt <- matrix(0,n,1)
    for(i in 2:n) dyt[i] <- (a*dyt[i-1])+et[i] 

    yo <- matrix(0,n,1)
    for(i in 2:n) yo[i]<-yo[i-1]+dyt[i]
    yo<-as.matrix(yo[2:n])
}
